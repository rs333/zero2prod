...
async fn spawn_app() -> TestApp {
    ...
    configuration.database.database_name = format!("{}{}", DB_PREFIX, Uuid::new_v4().to_string());
    ...
    let sender = configuration
        .email_client
        .sender()
        .expect("Invalid subscriber email address.");
...
async fn health_check_works() {
    ...
    let response = client
        .get(address.address + "/health_check")
        .send()
        .await
...
#[tokio::test]
async fn subscribe_returns_a_400_for_when_fields_are_present_but_empty() {
    // Arrange
    let app = spawn_app().await;
    let client = reqwest::Client::new();
    let test_cases = vec![
